...
class FilterTypeController():
    ...
    def apply_filter(self):
        ...
        if filter_name == "Butterworth Filter":
            zeroes, poles = self.butterworth_filter(filter_order, filter_cutoff_frequency, 'low')
            print(zeroes, poles)
        elif filter_name == "Chebyshev Filter":
            zeroes, poles = self.chebyshev_filter(filter_order, filter_cutoff_frequency, filter_passband_ripple, 'low')
            print(zeroes, poles)
        elif filter_name == "inv Chebyshev Filter":
            zeroes, poles = self.chebyshev_type_2_filter(filter_order, filter_cutoff_frequency, filter_stopband_ripple, 'low')
            print(zeroes, poles)
        elif filter_name == "Bessel Filter":
            zeroes, poles = self.bessel_filter(filter_order, filter_cutoff_frequency, 'low')
            print(zeroes, poles)
        elif filter_name == "Elliptic Filter":
            zeroes, poles = self.elliptic_filter(filter_order, filter_cutoff_frequency, filter_passband_ripple, filter_stopband_ripple, 'low')
            print(zeroes, poles)
